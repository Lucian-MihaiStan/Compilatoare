...
definition
    :
        (TYPE ID (ASSIGN expr)?)                                                # variable_def
        |
        (TYPE ID LPAREN (formal (COMMA formal)*)* RPAREN LBRACE expr RBRACE)    # function_def
	;
...
expr
    :
//        TYPE                                                        # type
        ID LPAREN (expr COMMA?)* RPAREN                                  # call
        | MINUS expr                                                  # minus
        | expr MULT expr                                      # mult
        | expr DIV expr                                      # div
        | expr PLUS expr                                    # plus
        | expr MINUS expr                                      # minus_expr
        | expr (LT | LE | EQUAL) expr                                 # op_rel
        | IF cond=expr THEN thenBranch=expr ELSE elseBranch=expr FI	# if
        | ID ASSIGN expr                                          # assign
        | LPAREN expr RPAREN                          # expr_par
        | ID                                                        # id
        | INT                                                       # int
        | FLOAT                                                     # float
        | BOOL                                                      # bool

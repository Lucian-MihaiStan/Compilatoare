public class ResolutionPassVisitor implements ASTVisitor<Void> {
    ...
    public Void visit(Assign assign) {
        ...
        Symbol symbol = assign.id.getScope().lookup(assign.id.getToken().getText());
    ...
    public Void visit(Call call) {
        ...
        Symbol symbol = call.id.getScope().lookup(call.id.getToken().getText());

        if (symbol == null) {
            ASTVisitor.error(call.getToken(), call.getToken().getText() + " function undefined");
            return null;
        }

        if (!(symbol instanceof FunctionSymbol)) {
            ASTVisitor.error(call.id.getToken(), call.id.getToken().getText() + " is not a variable");
            // return null;
        }

    ...
    static CoolParserBaseVisitor<ASTNode> createBaseVisitor() {
        return new CoolParserBaseVisitor<>() {
            ...
            public ASTNode visitBody(CoolParser.BodyContext ctx) {
                return new RfBody(ctx.statements.stream().map(expr -> (RfExpression) visit(expr)).collect(Collectors.toList()), ctx.start);
            }
            ...
            public ASTNode visitNegation(CoolParser.NegationContext ctx) {
                return new RfBitNegExpression((RfExpression) visit(ctx.value), ctx.TILDA().getSymbol());
            }
            ...
            public ASTNode visitMethod(CoolParser.MethodContext ctx) {
                return new RfMethod(ctx.ID().getSymbol().getText(), ctx.TYPE().getSymbol().getText(), ctx.formalParams.stream().map(arg -> (RfArgument) visit(arg)).collect(Collectors.toList()), (RfExpression) visit(ctx.methodBody), ctx.start);
            }
            ...
            public ASTNode visitField(CoolParser.FieldContext ctx) {
                return new RfField(ctx.ID().getSymbol().getText(), ctx.TYPE().getSymbol().getText(), ctx.expr() != null ? (RfExpression) visit(ctx.expr()) : null, ctx.start);
            }
            ...
            public ASTNode visitClass(CoolParser.ClassContext ctx) {
                return new RfClass(ctx.TYPE(), ctx.feature().stream().map(feature -> (RfFeature) visit(feature)).collect(Collectors.toList()), ctx.start);
            }

            @Override
            public ASTNode visitIsVoid(CoolParser.IsVoidContext ctx) {
                return new RfIsVoidExpression((RfExpression) visit(ctx.value), ctx.ISVOID().getSymbol());
            }
            ...
            public ASTNode visitProgram(CoolParser.ProgramContext ctx) {
                return new RfProgram(ctx.class_().stream().map(c -> (RfClass) visit(c)).collect(Collectors.toList()), ctx.start);
            }
            ...
            public ASTNode visitLet(CoolParser.LetContext ctx) {
                return new RfLet(ctx.decVars.stream().map(var -> new RfLet.RfDeclareVariable(
                        new RfId(var.ID().getSymbol().getText(), var.ID().getSymbol()),
                        var.TYPE().getSymbol().getText(),
                        var.initialValue == null ? null : (RfExpression) visit(var.initialValue),
                        var.COLON().getSymbol()
                )).collect(Collectors.toList()), (RfExpression) visit(ctx.letBody), ctx.LET().getSymbol());
            }

            @Override
            public ASTNode visitDispatch(CoolParser.DispatchContext ctx) {
                return new RfDispatch((RfExpression) visit(ctx.obj), ctx.TYPE() != null ? ctx.TYPE().getText() : null, new RfId(ctx.ID().getSymbol().getText(), ctx.ID().getSymbol()), ctx.params.stream().map(param -> (RfExpression) visit(param)).collect(Collectors.toList()), ctx.DOT().getSymbol());
            }

            @Override
            public ASTNode visitCaseBranch(CoolParser.CaseBranchContext ctx) {
                return new RfCase.RfCaseBranch(new RfId(ctx.ID().getSymbol().getText(), ctx.ID().getSymbol()), ctx.TYPE().getText(), (RfExpression) visit(ctx.branchBody), ctx.RESULTS_CASE().getSymbol());
            }

            @Override
            public ASTNode visitCase(CoolParser.CaseContext ctx) {
                return new RfCase((RfExpression) visit(ctx.cond), ctx.branches.stream().map(branch -> (RfCase.RfCaseBranch) visit(branch)).collect(Collectors.toList()), ctx.CASE().getSymbol());
            }
            ...
            public ASTNode visitImplicitDispatch(CoolParser.ImplicitDispatchContext ctx) {
                return new RfImplicitDispatch(new RfId(ctx.ID().getSymbol().getText(), ctx.ID().getSymbol()), ctx.params.stream().map(expr -> (RfExpression) visit(expr)).collect(Collectors.toList()), ctx.ID().getSymbol());
            }
